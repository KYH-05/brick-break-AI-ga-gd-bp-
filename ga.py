...
def initialize():
    ...
    global d_n
    d_n = []
    d_n = [[(random.uniform(-1, 1)) for k in range(0, weight_num)] for i in range(0, p_size)]
    # 지역 최적해
        ...
        for i in range(0, p_size):  ###################
            if dy < 0 and signal6 == 1:
                for i in range(0, p_size):
                    ...
                    moving[i] = 0
                signla6 = 0
            if (x > paddles[i].x - 10 and x < paddles[i].x + 23.45) or (
                    x >= paddles[i].x + 93.8 and x < paddles[i].x + 117.25 + 10):
                if dx < 0:
                    dx = -7.26
                elif dx > 0:
                    dx = 7.26
                signal5 = 1
            elif (x >= paddles[i].x + 23.45 and x < paddles[i].x + 46.9) or (
                    x >= paddles[i].x + 70.35 and x < paddles[i].x + 93.8):
                if dx < 0:
                    dx = -8.056
                elif dx > 0:
                    dx = 8.056
